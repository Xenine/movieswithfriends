                ...
                for (var ps = 0; ps < keys.length; ps++) {
                    for (var pp = 0; pp < kb_res.length; pp++) {
                        if (
                            keys[ps]
                                .toLowerCase()
                                .indexOf(kb_res[pp].toLowerCase()) + 1
                        ) {
        ...
        for (var i = 0, len = keys.length; i < len; i++) {
            ...
            if (
                ...
                if (btns.hasOwnProperty(key) && btns[key]) {
                    var q = players[key].quality
                        ? players[key].quality
                            ...
                            : players[key].quality
                                  .toUpperCase()
                                  .replace(
                                      /\s?ХОРОШЕЕ\s?|\s?СРЕДНЕЕ\s?|\s?ПЛОХОЕ\s?/gi,
                                      ''
                                  )
                        : ''
                    ...
                    j++
                    option.innerText =
                        j + '. ' + language.lookbase.toUpperCase()
                } else {
                    j++
    ...
    if (kinobdButtons) {
        ...
        setTimeout(function () {
            var btn = setInterval(function () {
                if (
                    parseInt(
                        (kinobdButtons.style && kinobdButtons.style.right) ||
                            '0'
                    ) >
    ...
    KbXmlHttp.onreadystatechange = function () {
        if (KbXmlHttp.readyState === 4) {
            if (KbXmlHttp.status === 200) {
                callback(
                    kb_json(KbXmlHttp.responseText),
                    KbXmlHttp.responseText
                )
            } else {
                callback({}, '')
            }
...
function kb_ping(provider, loadtime = null) {
    ...
    var inid = player.dataset.inid
        ? encodeURIComponent(player.dataset.inid)
        : ''
